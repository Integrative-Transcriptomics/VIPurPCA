...
def get_equipotential_samples(n, M, C):
    S = equipotential_standard_normal(M.shape[0] * M.shape[1], n)
    L, lower = jax.scipy.linalg.cho_factor(C, lower=True)
    M_flat, M_unflattener = flatten_util.ravel_pytree(M)
    samples = np.transpose(np.transpose(np.dot(L, S))+M_flat)
    return samples
